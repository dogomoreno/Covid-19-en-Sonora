...
```{r, include=FALSE}
POBMUN <- read_csv("C:/Users/luism/OneDrive/R/COVID/Sonora_Federal/Bases/POBMUN.csv", locale = locale(encoding = "ISO-8859-1"), col_types = cols(CVEGEO = col_character()))
Sonora.DF <- read_csv("C:/Users/luism/OneDrive/R/COVID/Sonora_Federal/Bases/ST_SonoraReporte_SSFED.csv", 
    col_types = cols(fecha_reporte = col_date(format = "%Y-%m-%d"))) %>% rename(Fecha=fecha_reporte)
Sonora.DF <- filter(Sonora.DF, Fecha>as.Date("2020-10-08"))
...
Hospitalizaciones <- read_csv("http://archivos.serendipiadata.com/ocupacion/concentrado/ocupacion_hospitalaria_Entidad_sin_ceros.csv", 
                                        ...
                                        locale = locale()) %>% filter(Estado=="Sonora") %>% rename(Fecha=FECHA)
...
-----------------------------------------------------------------------
```{r, include=FALSE}
CasosSonora = function(...) return(Sonora.dia$Casos)
...
CasosSon <- ggplot(Sonora.DF) +
  ...
  geom_line(aes(x= Fecha, y= Casos.media.7d, color= "Promedio móvil 7d"), linetype= "solid", size=0.5)+
  geom_point(aes(x= Fecha, y= Casos.diarios, fill= "Casos diarios"), color = "white", size = 1.5, stroke=0.5, alpha=0.6) +
   ...
   scale_color_manual(name="", values= c("Promedio móvil 7d" = "#01787E")) +
      scale_y_continuous(expand = c(0, 5), limits=c(0, max(Sonora.DF$Casos.diarios)+5)) +
...
DecesosSon <- ggplot(Sonora.DF) +
  ...
  geom_line(aes(x= Fecha, y= Decesos.media.7d, color= "Promedio móvil 7d"), linetype= "solid", size=0.5)+
  geom_point(aes(x= Fecha, y= Decesos.diarios, fill= "Decesos diarios"), color = "white", size = 1.5, stroke=0.5, alpha=0.6) +
   ...
   scale_color_manual(name="", values= c("Promedio móvil 7d" = "#73264D")) +
      scale_y_continuous(expand = c(0, 0), limits=c(0, max(Sonora.DF$Decesos.diarios)+5)) +
      scale_x_date(expand=c(0,5), date_breaks = "3 month", date_labels = "%B \n %Y") +
       theme_minimal() +
    theme(
    legend.position = "none",
...
Gravgraf <- ggplot(Sonora.Hosp) +
  ...
  geom_line(aes(x= Fecha, y= `% Ocupación`), color = "#E26B0A", size= 0.5, alpha=0.7) +
    scale_fill_manual(name="", values= c("% Ocupación" = "#FABF8F", "Graves" = "#E26B0A", "Críticos" = "#974706"  )) +
...
Sonora.pruebas <-Sonora.DF %>%  
  filter(Fecha > as.Date('2020-10-08'))
Pruebasdiarias <- ggplot(Sonora.pruebas) +
  geom_col(aes(x= Fecha, y= Resultados.diarios, fill= "Pruebas diarias"), size= 0.2) +
  geom_line(aes(x= Fecha, y= Resultados.media.7d, color= "Promedio móvil 7d"), linetype= "solid", size=0.5)+
   scale_fill_manual(name="", values= c("Pruebas diarias" = "#4BACC6")) + 
   scale_color_manual(name="", values= c("Promedio móvil 7d" = "#31869B")) +    
  scale_y_continuous(expand = c(0, 40), limits=c(0, max(Sonora.pruebas$Resultados.diarios))) +
      scale_x_date(expand=c(0,5), date_breaks = "3 month", date_labels = "%B \n %Y") +
       theme_minimal() +
    theme(
    legend.position = "none",
...
Decesossemana <- Decesos %>% group_by(MUNICIPIO) %>% 
  mutate(diasemana = weekdays(Fecha), decesossemana = NUEVOS) %>% 
  filter(diasemana==weekdays(max(as.Date(Fecha)))) %>% 
  left_join(POBMUN, by = "CVEGEO") 
...
```{r}
CDSELECT<- Mapa %>% select("MUNICIPIO","POBLACION", "CASOS", "CASOS NUEVOS", "DECESOS", "DECESOS NUEVOS","INCIDENCIAACUM", "MORTALIDADACUM", "LETALIDAD") %>% rename("INCIDENCIA\nACUMULADA"=INCIDENCIAACUM, "MORTALIDAD\nACUMULADA" =MORTALIDADACUM, "CASOS\nSEMANA"= "CASOS NUEVOS", "DECESOS\nSEMANA" = "DECESOS NUEVOS")
...
react2 <-reactable(CDSELECT4,searchable = FALSE, highlight = FALSE, defaultSorted = "CASOS", defaultSortOrder = "desc", defaultPageSize = nrow(CDSELECT4),
  ...
  defaultColDef = colDef(footerStyle = list(fontWeight = "bold"))) %>% reactablefmtr::add_title(font_size = 38, font_family = "Lato Black", "Reporte acumulado Covid-19 en Sonora") %>% reactablefmtr::add_subtitle(font_size = 20, font_family = "Lato", paste0("Acumulados al corte del ", day(dia),"/", month(dia), "/",year(dia))) %>%  reactablefmtr::add_source(font_size = 14, font_family = "Lato Light", "Elaboración Luis armando Moreno (@dogomoreno) con información de la Secretaría de Salud Federal.|            www.luisarmandomoreno.com")
