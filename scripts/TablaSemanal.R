...
Casos <- read_csv("C:/Users/luism/OneDrive/R/COVID/Sonora_Federal/Bases/Casosdiarios_SSFED.csv", 
  ...
  group_by(MUNICIPIO) %>% 
  mutate(Casossemana =  (CASOS - lag(CASOS, default = 0, order_by=Fecha)))
...
casosacumdia3 <- rename(casosacumdia,'CASOS NUEVOS'=Casossemana)
Semanal <-left_join(casosacumdia3, decesosacumdia3,by = c("CVEGEO","MUNICIPIO"))


CDSELECT<- Semanal %>% select("MUNICIPIO", "CASOS NUEVOS", "DECESOS NUEVOS") %>% 
...
gt1 = CDSELECT2 %>% 
  ...
  tab_header(title=md("<span style = 'font-size:12pt'> Covid-19 en Sonora</span><br> Reporte semanal      <img src='https://www.luisarmandomoreno.com/wp-content/uploads/2022/04/cropped-header.png' style='height:40px;'>"),
             subtitle=md(paste0("Variación semanal en el acumulado, corte al ", day(dia),"/", month(dia), "/",year(dia)))) %>%
