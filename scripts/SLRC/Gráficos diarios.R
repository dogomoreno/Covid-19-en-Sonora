...
SLRC.DF.hoy <- filter(SLRC.DF, fecha_reporte == dia)
...
SLRC.DF.hoy <- filter(SLRC.DF, fecha_reporte == max(fecha_reporte))
SLRC.DF.hoy <- select(SLRC.DF.hoy, Hospitalizados.Activos, Ambulatorios.Activos, Decesos, Recuperados)
SLRC.DF.hoy <- rename(SLRC.DF.hoy, "Ambulatorios activos"= Ambulatorios.Activos, "Hospitalizados Activos"=Hospitalizados.Activos)
SLRC.DF.hoy <- gather(SLRC.DF.hoy, key= Estatus, value= Casos.confirmados) 
...
ggsave("Gráficos diarios/SLRC/Diarioestatus.png",Estatus , width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)
...
SLRC.DF <- rename(SLRC.DF, Fecha=fecha_reporte)
SLRC.DF <-  filter(SLRC.DF, Fecha>as.Date("2020-10-08"))
SLRC.DF <- mutate(SLRC.DF, Casos.diarios= Casos - lag(Casos, default = Casos[1], order_by=Fecha))
SLRC.DF <- mutate(SLRC.DF, Confirmados.diarios= Confirmados_lab - lag(Confirmados_lab, default = Confirmados_lab[1], order_by=Fecha))
SLRC.DF <- mutate(SLRC.DF, Decesos.diarios= Decesos - lag(Decesos, default = Decesos[1], order_by=Fecha))
SLRC.DF <- mutate(SLRC.DF, Casos.media.7d=round(rollmeanr(x=Casos.diarios, 7, fill = NA),1))
SLRC.DF <- mutate(SLRC.DF, Decesos.media.7d=round(rollmeanr(x=Decesos.diarios,7, fill=NA),1))
SLRC.DF <- mutate(SLRC.DF, Resultados.diarios= Resultados_lab - lag(Resultados_lab, default = Resultados_lab [1]))
SLRC.DF <- mutate(SLRC.DF, Resultados.media.7d=round(rollmeanr(x=Resultados.diarios,7, fill=NA),1))
SLRC.DF <- mutate(SLRC.DF,Ambulatorios.Activos.7d=round(rollmeanr(x=Ambulatorios.Activos,7, fill=NA),1))
SLRC.DF <- mutate(SLRC.DF, Incidencia= round((Casos / 30.74745),2))
SLRC.DF <- mutate(SLRC.DF, Letalidad= round((Decesos / Casos)*100,1))
SLRC.DF <- mutate(SLRC.DF, Mortalidad= round((Decesos / 30.74745)*100,2))
SLRC.DF <- mutate(SLRC.DF, Positividad.diaria= round((Confirmados.diarios / Resultados.diarios)*100,2))
SLRC.DF <- mutate(SLRC.DF, Positividad= round((Confirmados_lab / Resultados_lab )*100,2))

estata.hoy <- SLRC.DF %>% filter(Fecha==max(as.Date(Fecha)))
estata.semana <- SLRC.DF %>% filter(Fecha==(max(as.Date(Fecha))-7))


# Casos diarios Estatal
CasosSLRC <- ggplot(SLRC.DF) +
  ...
  scale_y_continuous(expand = c(0, 0), limits=c(0, max(SLRC.DF$Casos.diarios)+5)) +
  ...
  theme(legend.text = element_text(family = "Lato", size = 7), legend.background = element_rect(fill="transparent"),
        ...
        legend.key = element_rect(fill="transparent", color="transparent")) +
  labs(y = NULL, 
       x = NULL,legend= NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en SLRC:</span><br><span style = 'color:#01A2AC';>Casos confirmados diariamente</span>", 
       subtitle= Fechahoy, caption =fuente)

CasosSLRC

ggsave("Gráficos diarios/SLRC/diariocasos.png",CasosSLRC, width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)

  
  DecesosSon <- ggplot(SLRC.DF) +
  ...
  scale_x_date(expand=c(0,0), limits = c(as.Date("2020-10-08"), max(as.Date(SLRC.DF$Fecha))+5), date_breaks = "1 month", date_labels = "%B") +
  ...
  theme(legend.text = element_text(family = "Lato", size = 7), legend.background = element_rect(fill="transparent"),
        ...
        legend.key = element_rect(fill="transparent", color="transparent")) +
  labs(y = NULL, 
       x = NULL,legend= NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en SLRC:</span><br><span style = 'color:#993366';>Decesos confirmados diariamente</span>", 
       subtitle= Fechahoy, caption =fuente)

DecesosSon

ggsave("Gráficos diarios/SLRC/diariodecesos.png",DecesosSon,  width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)

PruebasSon <- ggplot(SLRC.DF) +
  ...
  scale_y_continuous(expand=c(0,0), limits=c(0, max(SLRC.DF$Resultados.diarios,na.rm=TRUE)+20)) +
  ...
  theme(legend.text = element_text(family = "Lato", size = 7), legend.background = element_rect(fill="transparent"),
        ...
        legend.key = element_rect(fill="transparent", color="transparent")) +
  labs(y = NULL, 
       x = NULL,legend= NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en SLRC:</span><br><span style = 'color:#4BACC6';>Resultados informados diariamente</span>", 
       subtitle= Fechahoy, caption =fuente)

PruebasSon

ggsave("Gráficos diarios/SLRC/diariopruebas.png",PruebasSon, width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)




library(directlabels)
Letalidad <- SLRC.DF %>% ggplot(aes(x= Fecha, y= Letalidad)) +
  geom_line(color= "#993366", linetype= "solid", size=1, alpha=0.6)+
  geom_point( data = subset(SLRC.DF , Fecha == max(Fecha)), fill="#993366", size=2 , shape=21, color="white", stroke=1) +
  geom_dl( data = subset(SLRC.DF , Fecha == max(Fecha)), aes(label = Letalidad), color="#993366", method = list(dl.trans(x = x + 0.2), "last.bumpup", cex = 1.5, fontfamily= "Lato Black")) +
  scale_y_continuous(expand = c(0, 0), limits= c(0,20), breaks=seq(0,20,2)) +
  scale_x_date(expand=c(0,0), limits = c(as.Date("2021-01-01"), max(SLRC.DF$Fecha)+40), date_breaks = "1 month", date_labels = "%B") +
  coord_cartesian(expand = FALSE, clip = 'off') +
  theme_bw() +
  temaejes +
  labs(y = "Decesos por cada 100 casos", 
       x = NULL,legend= NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en SLRC:</span><br><span style = 'color:#993366';>Letalidad acumulada</span>", 
       subtitle= Fechahoy, caption =fuente)  

Letalidad

ggsave("Gráficos diarios/SLRC/s13.png",Letalidad,  width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)


#Hospitalizados

Hospitalizados <- SLRC.DF %>% ggplot(aes(x= Fecha, y= Hospitalizados.Activos)) +
  geom_line(color= "#F79646", linetype= "solid", size=1, alpha=0.8)+
  geom_area(color= "transparent", fill= "#F79646",alpha=0.1)+
  geom_point( data = subset(SLRC.DF , Fecha == max(Fecha)), fill="white", size=1 , shape=21, color="#F79646", stroke=1) +
  geom_dl( data = subset(SLRC.DF , Fecha == max(Fecha)), aes(label = Hospitalizados.Activos), color="#F79646", method = list(dl.trans(x = x + 0.2), "last.bumpup", cex = 1.5, fontfamily= "Lato Black")) +
  scale_y_continuous(expand = c(0, 0), limits= c(0,20), breaks=seq(0,20,5)) +
  scale_x_date(expand=c(0,0), limits = c(as.Date("2021-01-01"), max(as.Date(SLRC.DF$Fecha))+20), date_breaks = "1 month", date_labels = "%B") +
  coord_cartesian(expand = FALSE, clip = 'off') +
  theme_bw() +
  temaejes +
  labs(y = NULL, 
       x = NULL,legend= NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en SLRC:</span><br><span style = 'color:#F79646';>Casos activos hospitalizados al corte</span>", 
       subtitle= Fechahoy, caption =fuente)  

Hospitalizados

ggsave("Gráficos diarios/SLRC/Hospitalizados 2.png",Hospitalizados,  width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)



Activos <- SLRC.DF %>% ggplot() +
  ...
  geom_point( data = subset(SLRC.DF , Fecha == max(Fecha)),aes(x= Fecha, y= Ambulatorios.Activos), fill="white", size=1 , shape=21, color="#58BCBC", stroke=1) +
  geom_dl( data = subset(SLRC.DF , Fecha == max(Fecha)), aes(x= Fecha, y= Ambulatorios.Activos, label = Ambulatorios.Activos), color="#58BCBC", method = list(dl.trans(x = x + 0.2), "last.bumpup", cex = 1.5, fontfamily= "Lato Black")) +
  ...
  scale_x_date(expand=c(0,0), limits = c(as.Date("2021-01-01"), max(as.Date(SLRC.DF$Fecha))+30), date_breaks = "1 month", date_labels = "%B") +
  coord_cartesian(expand = FALSE, clip = 'off') +
  theme_bw() +
  temaejes + 
  theme(legend.text = element_text(family = "Lato", size = 7), legend.background = element_rect(fill="transparent"),
        ...
        legend.key = element_rect(fill="transparent", color="transparent")) +
  labs(y = NULL, 
       x = NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en SLRC:</span><br><span style = 'color:#58BCBC';>Pacientes activos con síntomas leves al corte</span>", 
       subtitle= Fechahoy, caption =fuente)  

Activos
ggsave("Gráficos diarios/SLRC/diariosActivos.png",Activos,  width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)



# O2A <- SLRC.DF %>% filter(Etapa=="Ascenso", OLA=="B") 
...
# 
# O3A <- SLRC.DF %>% filter(Etapa=="Ascenso", OLA=="C") 
...
#         legend.key = element_rect(fill="transparent", color="transparent")) +
#   labs(y = NULL, 
#        x = "Días desde el inicio de la ola", legend= NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en SLRC:</span><br>Casos acumulados confirmados en las etapas de<br>ascenso de la <span style = 'color:#01787E';>primera</span> y <span style = 'color:#F79646';>segunda</span> ola</span>", 
#        subtitle= Fechahoy, caption =fuente)
# 
# OLASG
# 
