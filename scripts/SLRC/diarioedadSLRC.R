...
Casos <- SLRC.DF %>% select(fecha_reporte, C_0_14, C_15_29,C_30_59,C_60_mas) %>% 
...
SLRC.Sintomas <- SLRC.Sintomas %>% select(Fecha, C_0_14, C_15_29,C_30_59,C_60_mas) %>% 
  select(Fecha, C_0_14, C_15_29,C_30_59,C_60_mas) %>% 
  rename ("0 a 14 años"=C_0_14, "15 a 29 años"=C_15_29,"30 a 59 años"=C_30_59,"60 o más años"=C_60_mas) %>% 
  gather(key= "grupoedad", value= "Casos", ends_with("años")) 

SLRC.Sintomas <- SLRC.Sintomas %>% 
  group_by(grupoedad) %>% 
...
CasosSLRC <- ggplot(SLRC.Sintomas) +
  geom_area(data=subset(SLRC.Sintomas, Fecha <= dia.act), aes( x= Fecha, y= Casos.media.7d), fill= "#58BCBC", alpha=0.3)+
  # geom_hline(yintercept=max(SLRC.Sintomas$Casos.semana), linetype="dashed", color = "gray80") +
  # geom_hline(yintercept=max(SLRC.Sintomas$Casos.hoy), linetype="dashed", color = "red") +
  geom_line(data=subset(SLRC.Sintomas, Fecha <=dia.act), aes(x= Fecha, y= Casos.media.7d, color= "Tendencia promedio móvil 7 días"), linetype= "solid", size=.55, arrow=arrow(type="open", length=unit(0.10,"cm")))+
  geom_point(data=subset(SLRC.Sintomas, Fecha <=dia.act), aes(x= Fecha, y= Casos,fill="Casos que iniciaron síntomas el día correspondiente"), color = "white", size = 0.5, stroke=0.4, alpha=0.65, shape = 21) +
  geom_area(data=subset(SLRC.Sintomas, Fecha >=dia.act), aes( x= Fecha, y= Casos.media.7d), fill= "#58BCBC", alpha=0.1)+
  geom_point(data=subset(SLRC.Sintomas, Fecha >dia.act), aes(x= Fecha, y= Casos), fill= "#01787E", color = "white", size = 0.5, stroke=0.4, alpha=0.30, shape = 21) +
  ...
  scale_x_date(expand=c(0,0),limits= c(min(SLRC.Sintomas$Fecha)-5,max(SLRC.Sintomas$Fecha)+10), date_breaks = "2 month", 
  ...
  labs(y = NULL, 
       x = NULL,legend= NULL, title  = paste0("<span style = 'font-size:14pt'>Covid-19 en SLRC</span><br><span style = 'color:#01A2AC';>Casos confirmados por grupo de edad</span>"), 
       subtitle= Fechahoy, caption =fuentefech)

CasosSLRC

ggsave("Gráficos diarios/SLRC/diariocasosedadSLRC.png",CasosSLRC, width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)
...
Decesos <- SLRC.DF %>% select(fecha_reporte, D_0_14, D_15_29,D_30_59,D_60_mas) %>% 
...
SLRC.Def <- read_csv("Bases/ST_SLRCDefuncion_SSFED.csv", 
                            col_types = cols(fecha_def = col_date(format = "%Y-%m-%d"))) %>% rename(Fecha=fecha_def)

SLRC.Def <- SLRC.Def %>% select(Fecha, D_0_14, D_15_29,D_30_59,D_60_mas) %>% 
...
DecesosSLRC <- ggplot(SLRC.Defuncion) +
  geom_area(data=subset(SLRC.Defuncion, Fecha <=dia.act), aes( x= Fecha, y= Decesos.media.7d), fill= "#D075A3", alpha=0.3)+
  geom_line(data=subset(SLRC.Defuncion, Fecha <=dia.act), aes(x= Fecha, y= Decesos.media.7d, color= "Tendencia promedio móvil 7 días"), linetype= "solid", size=.45, arrow=arrow(type="open", length=unit(0.10,"cm")))+
  geom_point(data=subset(SLRC.Defuncion, Fecha <=dia.act), aes(x= Fecha, y= Decesos,fill="Decesos ocurridos el día correspondiente"), color = "white", size = 0.4, stroke=0.4, alpha=0.65, shape = 21) +
  geom_area(data=subset(SLRC.Defuncion, Fecha >=dia.act), aes( x= Fecha, y= Decesos.media.7d), fill= "#D075A3", alpha=0.15)+
  geom_point(data=subset(SLRC.Defuncion, Fecha >dia.act), aes(x= Fecha, y= Decesos), fill= "#73264D", color = "white", size = 0.4, stroke=0.4, alpha=0.20, shape = 21) +
  ...
  labs(y = NULL, 
       x = NULL,legend= NULL, 
       title  = paste0("<span style = 'font-size:14pt'>Covid-19 en SLRC</span><br><span style = 'color:#993366';>Decesos confirmados por grupo de edad</span>"), 
       subtitle= Fechahoy, caption =fuentefech)

DecesosSLRC

ggsave("Gráficos diarios/SLRC/diariodecesosedadSLRC.png",DecesosSLRC, width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)
