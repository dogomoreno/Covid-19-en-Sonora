...
SINTOMAS <- read_csv("Bases/COVIDHermosillo_SINTOMAS.csv", 
...
Hospitalizados <- sintomassemana  %>%
  ...
  scale_x_date(expand=c(0,0), limits = c(as.Date("2021-01-01"), max(as.Date(sintomassemana$fecha_sintomas)+30)),
               date_breaks = "1 month", labels = function(x) if_else(is.na(lag(x)) | !year(lag(x)) == year(x), 
                                                                     paste(month(x, label = TRUE), "\n", year(x)),
                                                                     paste(month(x, label = TRUE)))) +
  coord_cartesian(expand = FALSE, clip = 'off') +
