...
rm(list=ls())
fuentenyt <- "*Casos por fecha de inicio de síntomas, y decesos por fecha de ocurridos.\nElaboración Luis Armando Moreno (@sonoraendatos) siguiendo el código de @theneilrichards inspirado en The New York Times\ncon información de la Secretaría de Salud del Gobierno de la República y la Secretaría de Salud Estatal\n*Por fecha de reporte | www.sonoraendatos.com"

Sonora.EF <- read_csv("Bases/ST_SonoraReporte_SSFED.csv", 
                      col_types = cols(fecha_reporte = col_date(format = "%Y-%m-%d"))) %>% rename(Fecha=fecha_reporte) %>% filter(!is.na(Fecha))

dia<-max(as.Date(Sonora.EF$Fecha))


Fechahoy <- paste0("Al reporte del ", day(dia), " de ", months.Date(dia)," de ", year(dia),".")
fuente <- "Elaboración Luis Armando Moreno (@sonoraendatos) con información de la Secretaría de Salud del Gobierno de la República\n*Por fecha de reporte | www.sonoraendatos.com"
fuenteprueba <- "Elaboración Luis Armando Moreno (@sonoraendatos) con información de la Secretaría de Salud del Gobierno de la República\n*Incluye resultados válidos de pruebas PCR y antigénicas. | www.sonoraendatos.com"
fuenteact <- "Elaboración Luis Armando Moreno (@sonoraendatos) con información de la Secretaría de Salud del Gobierno de la República\n*Se consideran casos activos aquellos que iniciaron síntomas dentro de los 14 días previos al corte. | www.sonoraendatos.com"
...
Sonora.DF <- read_csv("Bases/ST_SonoraSintomas_SSFED.csv", 
                      col_types = cols(fecha_sintomas = col_date(format = "%Y-%m-%d"))) %>% rename(Fecha=fecha_sintomas)
Sonora.DF <- mutate(Sonora.DF, Casos.diarios= Casos)

Sonora.DF <- select(Sonora.DF, Fecha, Casos.diarios)

estata.hoy <- Sonora.EF %>% filter(Fecha==max(as.Date(Fecha)))
estata.semana <- Sonora.EF %>% filter(Fecha==(max(as.Date(Fecha))-7))


#Casos diarios Estatal

Sonora.ED <- read_csv("Bases/ST_SonoraDefuncion_SSFED.csv", 
                      col_types = cols(fecha_def = col_date(format = "%Y-%m-%d"))) %>% rename(Fecha=fecha_def)
Sonora.ED <- mutate(Sonora.ED, Decesos.diarios= Decesos)
Sonora.ED <- select(Sonora.ED, Fecha, Decesos.diarios)


Sonora.ED <- Sonora.DF %>% left_join(Sonora.ED, by=c("Fecha")) %>% arrange(Fecha) %>% mutate_if(is.numeric,coalesce,0) %>% 
...
covid_cases <- Sonora.ED %>%
  ...
  filter(Fecha != as_date("2020-02-29"))   %>% arrange(Fecha) %>% 
...
month_breaks <- c(1, 32, 60, 91, 121, 152,
...
p_legend <- 
  ...
  labs(title = "<span style = 'font-size:8pt'>Covid-19 en Sonora</span><br><span style = 'font-size:18pt; color:#01A2AC';>Casos</span> y <span style = 'font-size:18pt;color:#993366';>decesos</span>",
       subtitle="Promedio móvil a 7 días<br>") +
  theme_void() +
  theme(plot.title = element_markdown(color = text_color, 
                                      family = "Lato Black",
                                      face = "bold", size = 10,
...
fuentenyt2 <- "Elaboración Luis Armando Moreno (@sonoraendatos) siguiendo el código de @VictimOfMaths inspirado en The New York Times\ncon información de la Secretaría de Salud del Gobierno de la República y la Secretaría de Salud Estatal\n*Por fecha de reporte | www.sonoraendatos.com"
