...
Sonora.DF.hoy <- select(Sonora.DF.hoy, Hospitalizados.Activos, Ambulatorios.Activos, Decesos, Recuperados)
Sonora.DF.hoy <- rename(Sonora.DF.hoy, "Amb act"= Ambulatorios.Activos, "Hospitalizados activos"=Hospitalizados.Activos)
Sonora.DF.hoy <- gather(Sonora.DF.hoy, key= Estatus, value= Casos.confirmados) 
...
Casossemana <- Casos %>% group_by(MUNICIPIO) %>% 
  mutate(diasemana = weekdays(Fecha), Casossemana = NUEVOS) %>% 
...
CasosSon <- ggplot(Sonora.ED) +
  ...
  theme(legend.text = element_text(family = "Lato", size = 7), legend.background = element_rect(fill="transparent"),
        ...
        legend.key = element_rect(fill="transparent", color="transparent")) +
  labs(y = NULL,
       x = NULL,legend= NULL, title  = paste0("<span style = 'font-size:14pt'>Covid-19 en Sonora</span><br><span style = 'color:#01A2AC';>Casos confirmados acumulados: ",prettyNum(as.numeric(sum(Sonora.DF.hoy$Casos.confirmados)), big.mark=",", preserve.width="none"),"</span>"),
...
ggsave("Gráficos diarios/diariocasosreporte.png",CasosSon, width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)


dias <- c("lunes", "martes", "miércoles", "jueves", "viernes", "sábado", "domingo")
Sonora.semana <- Sonora.ED %>% mutate(semana=isoweek(Fecha), diasemana=weekdays(Fecha)) %>% mutate(diasemana=factor(diasemana, dias)) %>% filter(Fecha>as.Date("2020-03-16"))
Sonora.semana %>% ggplot()+
  geom_line(aes(x=Fecha, y=Casos.diarios), color="#01A2AC") +
  scale_y_continuous(expand = c(0,0), limits = c(0, max(Sonora.semana$Casos.diarios))) +
  ...
  labs(y = NULL,
       x = NULL,legend= NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en Sonora</span><br><span style = 'color:#01A2AC';>Casos confirmados por día de la semana</span>",
       subtitle= paste0(Fechahoy), caption =fuenteimp)
ggsave("Gráficos diarios/diariocasossemanarep.png", width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)

HospSon <- Sonora.DF %>% filter(Fecha>=as.Date("2021-07-01")) %>% ggplot() +
  geom_hline(yintercept=estata.semana$Hospitalizados.diarios, linetype="dashed", color = "gray65") +
  geom_text(aes(x = as.Date("2021-07-05"), y = (estata.semana$Hospitalizados.diarios + 2.5),
  ...
  scale_x_date(expand=c(0,1), date_breaks = "1 month", labels = function(x) if_else(is.na(lag(x)) | !year(lag(x)) == year(x), 
                                                                                    paste(month(x, label = TRUE), "\n", year(x)), 
                                                                                    paste(month(x, label = TRUE)))) +
  geom_text(aes(x = as.Date("2021-07-05"), y = (estata.hoy$Hospitalizados.diarios + 3),
  ...
  theme(legend.text = element_text(family = "Lato", size = 7), legend.background = element_rect(fill="transparent"),
        ...
        legend.key = element_rect(fill="transparent", color="transparent")) +
  labs(y = NULL,
       x = NULL,legend= NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en Sonora</span><br><span style = 'color:#F79646';>Casos confirmados hospitalizados por reporte</span>",
       subtitle= paste0("Variación en el acumulado de casos confirmados hospitalizados respecto al día anterior.\n", Fechahoy), caption =fuenteimp)

HospSon <- cowplot::ggdraw() +
  cowplot::draw_plot(HospSon,x = 0, y = 0, width = 1, height = 1) +
  cowplot::draw_image(logoH, x = 0.88, y = 0.89, width = 0.1, height = 0.1)  

ggsave("Gráficos diarios/Hospitalizados3.png",HospSon, width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)


Casos2olas <- Sonora.ED %>% filter(Fecha>=as.Date("2021-07-01")) %>% ggplot() +
  geom_hline(yintercept=estata.semana$Casos.diarios, linetype="dashed", color = "gray65") +
  geom_text(aes(x = as.Date("2021-07-05"), y = (estata.semana$Casos.diarios + 30),
                label = paste0("+", estata.semana$Casos.diarios, " casos el mismo día de la semana anterior")), stat = "unique", family = "Lato Black",
            size = 2, color = "gray65", hjust=0)+
  geom_hline(yintercept=estata.hoy$Casos.diarios, linetype="dashed", color = "red") +
  geom_area(aes(x= Fecha, y= Casos.media.7d), fill= "#58BCBC", alpha=0.3)+
  ...
  scale_x_date(expand=c(0,1), date_breaks = "1 month", labels = function(x) if_else(is.na(lag(x)) | !year(lag(x)) == year(x), 
                                                                                    paste(month(x, label = TRUE), "\n", year(x)), 
                                                                                    paste(month(x, label = TRUE)))) +
  geom_text(aes(x = as.Date("2021-07-05"), y = (estata.hoy$Casos.diarios + 30),
                label = paste0("+", estata.hoy$Casos.diarios, " casos hoy")), stat = "unique", family = "Lato Black",
            size = 3, color = "red",  hjust=0)+
  theme_bw() + temaejes +
  theme(legend.text = element_text(family = "Lato", size = 7), legend.background = element_rect(fill="transparent"),
...
Casos2olas <- cowplot::ggdraw() +
  cowplot::draw_plot(Casos2olas,x = 0, y = 0, width = 1, height = 1) +
  cowplot::draw_image(logo, x = 0.88, y = 0.89, width = 0.1, height = 0.1)  
ggsave("Gráficos diarios/olasdiariocasosreporte.png",Casos2olas, width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)

Decesos2olas <- Sonora.ED %>% filter(Fecha>=as.Date("2021-07-01")) %>% ggplot() +
  geom_hline(yintercept=estata.semana$Decesos.diarios, linetype="dashed", color = "gray65") +
  geom_text(aes(x = as.Date("2021-07-05"), y = (estata.semana$Decesos.diarios + 3),
  ...
  geom_line(aes(x= Fecha, y= Decesos.media.7d, color= "Tendencia promedio móvil 7 días"), linetype= "solid", size=.7, arrow=arrow(type="open", length=unit(0.10,"cm")))+
  geom_point(aes(x= Fecha, y= Decesos.diarios), color = "white", fill= "#73264D", size = 0.8, stroke=0.4, alpha=0.65, shape = 21) +
  ...
  scale_x_date(expand=c(0,1), date_breaks = "1 month", labels = function(x) if_else(is.na(lag(x)) | !year(lag(x)) == year(x), 
                                                                                     paste(month(x, label = TRUE), "\n", year(x)), 
                                                                                     paste(month(x, label = TRUE)))) +
  geom_text(aes(x = as.Date("2021-07-05"), y = estata.hoy$Decesos.diarios + 3,
  ...
  theme(legend.text = element_text(family = "Lato", size = 7), legend.background = element_rect(fill="transparent"),
        ...
        legend.key = element_rect(fill="transparent", color="transparent")) +
  labs(y = NULL,
       x = NULL,legend= NULL, title  = paste0("<span style = 'font-size:14pt'>Covid-19 en Sonora</span><br><span style = 'color:#993366';>Decesos confirmados acumulados: ",prettyNum(as.numeric(estata.hoy$Decesos), big.mark=",", preserve.width="none"),"</span>"),
       subtitle= paste0("Variación en el acumulado de decesos confirmados por fecha de reporte respecto al día anterior.\n", Fechahoy), caption =fuenteimp)

Decesos2olas <- cowplot::ggdraw() +
  cowplot::draw_plot(Decesos2olas,x = 0, y = 0, width = 1, height = 1) +
  cowplot::draw_image(logoD, x = 0.88, y = 0.89, width = 0.1, height = 0.1)  
ggsave("Gráficos diarios/Olasdiariodecesosrep.png",Decesos2olas,  width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)

  DecesosSon <- ggplot(Sonora.ED) +
  geom_hline(yintercept=estata.semana$Decesos.diarios, linetype="dashed", color = "gray65") +
...
DecesosSon <- cowplot::ggdraw() +
  cowplot::draw_plot(DecesosSon,x = 0, y = 0, width = 1, height = 1) +
  cowplot::draw_image(logoD, x = 0.88, y = 0.89, width = 0.1, height = 0.1)

ggsave("Gráficos diarios/diariodecesosrep.png",DecesosSon,  width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)
Sonora.DF <-  filter(Sonora.DF, Fecha>as.Date("2020-05-01"))
PruebasSon <- ggplot(Sonora.DF) +
  ...
  scale_y_continuous(expand=c(0,0), limits=c(0, max(Sonora.DF$Resultados.diarios,na.rm=TRUE)+500)) +
  scale_x_date(expand=c(0,10), date_breaks = "1 month", labels = function(x) if_else(is.na(lag(x)) | !year(lag(x)) == year(x), 
                                                                                     paste(month(x, label = TRUE), "\n", year(x)), 
                                                                                     paste(month(x, label = TRUE)))) +
  ...
  theme(legend.text = element_text(family = "Lato", size = 7), legend.background = element_rect(fill="transparent"),
        ...
        legend.key = element_rect(fill="transparent", color="transparent")) +
  labs(y = NULL, 
       x = NULL,legend= NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en Sonora</span><br><span style = 'color:#4BACC6';>Resultados válidos informados diariamente</span>", 
       subtitle=Fechahoy, caption =fuenteprueba)

PruebasSon  <- cowplot::ggdraw() +
  cowplot::draw_plot(PruebasSon,x = 0, y = 0, width = 1, height = 1) +
...
AnalizadosSon <- ggplot(Sonora.A) +
  ...
  scale_color_manual(name="", values= c("Tendencia promedio móvil 7 días" = "#31859C")) +
  scale_y_continuous(expand=c(0,0), limits=c(0, max(Sonora.A$Analizados.diarios,na.rm=TRUE)+300)) +
  ...
  theme(legend.text = element_text(family = "Lato", size = 7), legend.background = element_rect(fill="transparent"), legend.box = "horizontal",
        ...
        legend.key = element_rect(fill="transparent", color="transparent")) +
  labs(y = NULL, 
       x = NULL,legend= NULL, title  =paste0("<span style = 'font-size:14pt'>Covid-19 en Sonora:</span><br><span style = 'color:#4BACC6';>Personas analizadas: ", prettyNum(as.numeric(estata.hoy$Analizados), big.mark=",", preserve.width="none"),"</span>"), 
       subtitle= Fechahoy, caption =fuente)

AnalizadosSon <- cowplot::ggdraw() +
  cowplot::draw_plot(AnalizadosSon,x = 0, y = 0, width = 1, height = 1) +
  cowplot::draw_image(logoP, x = 0.88, y = 0.89, width = 0.1, height = 0.1)

ggsave("Gráficos diarios/diarioAnalizados.png",AnalizadosSon, width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)
...
Hospitalizados <- Sonora.DF %>% ggplot() +
  ...
  geom_dl( data = subset(Sonora.DF , Fecha == max(Fecha)), aes(x= Fecha, y= Hospitalizados.Activos+10, label = Hospitalizados.Activos), color="#F79646", method = list(dl.trans(x = x + 0.2), "last.bumpup", cex = 1.5, fontfamily= "Lato Black")) +
  scale_y_continuous(expand = c(0, 0), limits= c(0,300), breaks=seq(0,300,50)) +
  scale_x_date(expand=c(0,0), limits = c(as.Date("2020-05-01"), max(Sonora.DF$Fecha)+50), date_breaks = "1 month", 
               labels = function(x) if_else(is.na(lag(x)) | !year(lag(x)) == year(x), 
                                            paste(month(x, label = TRUE), "\n", year(x)), 
                                            paste(month(x, label = TRUE)))) +
  coord_cartesian(expand = FALSE, clip = 'off') +
...
Activos <- Sonora.DF %>% ggplot() +
  ...
  geom_dl( data = subset(Sonora.DF , Fecha == max(Fecha)), aes(x= Fecha, y= Ambulatorios.Activos+300, label = Ambulatorios.Activos), color="#58BCBC", method = list(dl.trans(x = x + 0.2), "last.bumpup", cex = 1.5, fontfamily= "Lato Black")) +
  scale_color_manual(values= c("Tendencia promedio móvil 7 días"= "#58BCBC"))+
  scale_y_continuous(expand = c(0, 0), limits= c(0,max(Sonora.DF$Ambulatorios.Activos)+500), breaks=seq(0,max(Sonora.DF$Ambulatorios.Activos)+500,500)) +
  scale_x_date(expand=c(0,0), limits = c(as.Date("2020-05-01"), max(Sonora.DF$Fecha)+60), date_breaks = "1 month", 
                labels = function(x) if_else(is.na(lag(x)) | !year(lag(x)) == year(x), 
                                             paste(month(x, label = TRUE), "\n", year(x)), 
                                             paste(month(x, label = TRUE)))) +
  coord_cartesian(expand = FALSE, clip = 'off') +
...
Sospechosos <- Sonora.DF %>% ggplot() +
  ...
  geom_point( data = subset(Sonora.DF , Fecha == max(Fecha)),aes(x= Fecha, y= Sospechosos), fill="white", size=1 , shape=21, color="#A96AC2", stroke=1) +
  geom_dl( data = subset(Sonora.DF , Fecha == max(Fecha)), aes(x= Fecha, y= Sospechosos, label = Sospechosos), color="#A96AC2", method = list(dl.trans(x = x + 0.2), "last.bumpup", cex = 1.5, fontfamily= "Lato Black")) +
  ...
  scale_x_date(expand=c(0,0), limits = c(as.Date("2020-05-01"), max(as.Date(Sonora.DF$Fecha))+60), date_breaks = "1 month", 
               labels = function(x) if_else(is.na(lag(x)) | !year(lag(x)) == year(x), 
                                            paste(month(x, label = TRUE), "\n", year(x)), 
                                            paste(month(x, label = TRUE)))) +
  coord_cartesian(expand = FALSE, clip = 'off') +
  ...
  theme(legend.text = element_text(family = "Lato", size = 7), legend.background = element_rect(fill="transparent"),
        ...
        legend.key = element_rect(fill="transparent", color="transparent")) +
  labs(y = NULL, 
       x = NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en Sonora</span><br><span style = 'color:#A96AC2';>Pacientes con sospecha al corte</span>", 
       subtitle= Fechahoy, caption =fuente)  

Sospechosos <- cowplot::ggdraw() +
  cowplot::draw_plot(Sospechosos,x = 0, y = 0, width = 1, height = 1) +
  cowplot::draw_image(logoS, x = 0.88, y = 0.89, width = 0.1, height = 0.1)

ggsave("Gráficos diarios/diariosSospechosos.png",Sospechosos,  width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)

SospechososD <- Sonora.DF %>% ggplot() +
  ...
  #geom_area(aes(x=Fecha, y=Sospechosos.7d),color= "transparent", fill= "#A96AC2",alpha=0.1)+
  geom_point( data = subset(Sonora.DF , Fecha == max(Fecha)),aes(x= Fecha, y= Sospechosos.diarios), fill="white", size=1 , shape=21, color="#A96AC2", stroke=1) +
  geom_dl( data = subset(Sonora.DF , Fecha == max(Fecha)), aes(x= Fecha, y= Sospechosos.diarios, label = Sospechosos.diarios), color="#A96AC2", method = list(dl.trans(x = x + 0.2), "last.bumpup", cex = 1.5, fontfamily= "Lato Black")) +
  scale_color_manual(values= c("Tendencia promedio móvil 7 días"= "#A96AC2"))+
  scale_y_continuous(expand = c(0, 0), limits= c(-200,200), breaks=seq(-200,200,50)) +
  scale_x_date(expand=c(0,0), limits = c(as.Date("2021-09-01"), max(as.Date(Sonora.DF$Fecha))+10), date_breaks = "1 month", date_labels = "%B") +
  coord_cartesian(expand = FALSE, clip = 'off') +
  theme_bw() +
  temaejes + 
  theme(legend.text = element_text(family = "Lato", size = 7), legend.background = element_rect(fill="transparent"),
        ...
        legend.key = element_rect(fill="transparent", color="transparent")) +
  labs(y = NULL, 
       x = NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en Sonora</span><br><span style = 'color:#A96AC2';>Variación de pacientes sospechosos al corte</span>", 
       subtitle= Fechahoy, caption =fuente)  

SospechososD  <- cowplot::ggdraw() +
  cowplot::draw_plot(SospechososD,x = 0, y = 0, width = 1, height = 1) +
  cowplot::draw_image(logoS, x = 0.88, y = 0.89, width = 0.1, height = 0.1)

ggsave("Gráficos diarios/diariosvarSospechosos.png",SospechososD,  width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)
...
Ocupacion <- read_csv("http://archivos.serendipiadata.com/ocupacion/concentrado/ocupacion_hospitalaria_Entidad_sin_ceros.csv",
                      ...
                      locale = locale())
Ocupacion <- Ocupacion %>% filter(Estado=="Sonora") %>% rename(Fecha=FECHA,Ocupacion= "% Ocupación") %>% filter(Tipo=="General") %>% mutate(Ocupacion=round(Ocupacion/100,3))
...
OcupH <- Ocupacion %>% ggplot() +
  ...
  scale_x_date(expand=c(0,0), limits = c(as.Date("2020-05-01"), max(Sonora.DF$Fecha)+70), date_breaks = "1 month",
               labels = function(x) if_else(is.na(lag(x)) | !year(lag(x)) == year(x),
                                            paste(month(x, label = TRUE), "\n", year(x)),
                                            paste(month(x, label = TRUE)))) +
  coord_cartesian(expand = FALSE, clip = 'off') +
  ...
  temaejes +
  labs(y = NULL,
       x = NULL,legend= NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en Sonora:</span><br><span style = 'color:#F79646';>Ocupación hospitalaria general al corte</span>",
       subtitle= Fechahoyocup, caption =fuente)

OcupH
"04-11-2021 32.05"

ggsave("Gráficos diarios/Hospitalizados 1.png",OcupH,  width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)
...
Sonora.Sintomas <- rename(Sonora.Sintomas, Fecha=fecha_sintomas)
Sonora.Sintomas <- mutate(Sonora.Sintomas, Casos.media.7d=round(rollmeanr(x=Casos, 7, fill = NA),1))
Sonora.Sintomas <- mutate(Sonora.Sintomas, Sospechosos.7d=round(rollmeanr(x=Sospechosos, 7, fill = NA),1))
Sonora.Sintomas <- mutate(Sonora.Sintomas, Sospechosos.Act=round(rollsum(Sospechosos, 13, align="right", fill = 0)))
Sonora.Sintomas <- filter(Sonora.Sintomas, Fecha>as.Date("2020-03-03"))

sintomas.hoy <- Sonora.Sintomas %>% filter(Fecha==max(as.Date(Fecha)))

# estata.sint.hoy <- Sonora.DF %>% filter(Fecha==max(as.Date(Fecha)))
...
CasosSon <- ggplot(Sonora.Sintomas) +
  ...
  geom_hline(yintercept=estata.hoy$Casos.diarios, linetype="dashed", color = "red") +
  geom_line(data=Sonora.ED, aes(x= Fecha, y= Casos.media.7d, color= "Tendencia promedio móvil 7 días por fecha de reporte"), lineend="round", linejoin="round",linetype= "solid", size=.55, arrow=arrow(type="open", length=unit(0.10,"cm")))+
  geom_line(data=subset(Sonora.Sintomas, Fecha <=dia.act), aes(x= Fecha, y= Casos.media.7d, color= "Tendencia promedio móvil 7 días por fecha de inicio de síntomas"), lineend="round", linejoin="round",linetype= "solid", size=.7, arrow=arrow(type="open", length=unit(0.10,"cm")))+
  geom_point(data=subset(Sonora.Sintomas, Fecha <=dia.act), aes(x= Fecha, y= Casos,fill="Casos que iniciaron síntomas el día correspondiente"), color = "white", size = 0.7, stroke=0.2,shape = 21) +
  geom_point(data=subset(Sonora.Sintomas, Fecha >dia.act), aes(x= Fecha, y= Casos), fill= alpha("#01787E", 0.45), color = "white", size = 0.7, stroke=0.2, alpha=0.45, shape = 21) +
...
SospechososSINT <- Sonora.Sintomas %>% ggplot() +
  ...
  #geom_point( data = subset(Sonora.Sintomas , Fecha == max(Fecha)),aes(x= Fecha, y= Sospechosos), fill="white", size=1 , shape=21, color="#A96AC2", stroke=1) +
  #geom_dl( data = subset(Sonora.Sintomas , Fecha == max(Fecha)), aes(x= Fecha, y= Sospechosos, label = Sospechosos), color="#A96AC2", method = list(dl.trans(x = x + 0.2), "last.bumpup", cex = 1.5, fontfamily= "Lato Black")) +
  ...
  scale_x_date(expand=c(0,0), limits = c(as.Date("2020-05-01"), max(as.Date(Sonora.DF$Fecha))+10), date_breaks = "1 month", 
               labels = function(x) if_else(is.na(lag(x)) | !year(lag(x)) == year(x), 
                                            paste(month(x, label = TRUE), "\n", year(x)), 
                                            paste(month(x, label = TRUE)))) +
  coord_cartesian(expand = FALSE, clip = 'off') +
  ...
  guides(fill = guide_legend(order = 1), 
         color = guide_legend(order = 2)) +
  labs(y = NULL, 
       x = NULL,legend= NULL, title  = paste0("<span style = 'font-size:14pt'>Covid-19 en Sonora</span><br><span style = 'color:#A96AC2';>Casos sospechosos al corte: ",prettyNum(as.numeric(estata.hoy$Sospechosos), big.mark=",", preserve.width="none"),"</span>"), 
       subtitle= Fechahoy, caption =fuentefech)

SospechososSINT <- cowplot::ggdraw() +
  cowplot::draw_plot(SospechososSINT  ,x = 0, y = 0, width = 1, height = 1) +
  cowplot::draw_image(logoS, x = 0.88, y = 0.89, width = 0.1, height = 0.1)
ggsave("Gráficos diarios/diariosSospechososSINT.png",SospechososSINT,  width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)


Sonora.Defuncion <- read_csv("Bases/ST_SonoraDefuncion_SSFED.csv", 
                             col_types = cols(fecha_def = col_date(format = "%Y-%m-%d")))

Sonora.Defuncion <- rename(Sonora.Defuncion , Fecha=fecha_def)
Sonora.Defuncion  <- mutate(Sonora.Defuncion , Decesos.media.7d=round(rollmeanr(x=Decesos,7, fill=NA),1))
Sonora.Defuncion <- filter(Sonora.Defuncion, Fecha>as.Date("2020-03-28"))

DecesosSon <- ggplot(Sonora.Defuncion) +
  geom_line(data=Sonora.ED, aes(x= Fecha, y= Decesos.media.7d, color= "Tendencia promedio móvil 7 días por fecha de reporte"),lineend="round", linejoin="round", linetype= "solid", size=.5, arrow=arrow(type="open", length=unit(0.10,"cm")))+
  ...
  geom_hline(yintercept=estata.hoy$Decesos.diarios, linetype="dashed", color = "red") +
  geom_line(data=subset(Sonora.Defuncion, Fecha <=dia.act), aes(x= Fecha, y= Decesos.media.7d, color= "Tendencia promedio móvil 7 días por fecha de ocurridos"),lineend="round", linejoin="round", linetype= "solid", size=.7, arrow=arrow(type="open", length=unit(0.10,"cm")))+
  geom_point(data=subset(Sonora.Defuncion, Fecha <=dia.act), aes(x= Fecha, y= Decesos,fill="Decesos ocurridos el día correspondiente"), color = "white", size = 0.6, stroke=0.4, alpha=0.65, shape = 21) +
  geom_point(data=subset(Sonora.Defuncion, Fecha >dia.act), aes(x= Fecha, y= Decesos), fill= "#73264D", color = "white", size = 0.6, stroke=0.2, shape = 21) +
  ...
  scale_y_continuous(expand = c(0, 0), limits= c(0,160)) +
  scale_x_date(expand=c(0,0), limits= c(min(Sonora.Defuncion$Fecha)-5,max(Sonora.Defuncion$Fecha)+10), date_breaks = "1 month", 
               labels = function(x) if_else(is.na(lag(x)) | !year(lag(x)) == year(x), 
                                            paste(month(x, label = TRUE), "\n", year(x)), 
                                            paste(month(x, label = TRUE)))) +
  guides(fill = guide_legend(order = 1), 
  ...
  labs(y = NULL, 
       x = NULL,legend= NULL, 
       title  = paste0("<span style = 'font-size:14pt'>Covid-19 en Sonora</span><br><span style = 'color:#993366';>Decesos confirmados acumulados: ",prettyNum(as.numeric(estata.hoy$Decesos), big.mark=",", preserve.width="none"),"</span>"), 
       subtitle= Fechahoy, caption =fuentefech)
